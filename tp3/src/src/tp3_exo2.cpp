...
cv::Mat LUTimageNormalize(const cv::Mat &image){
  ...
  for(int i=0; i<256; i++)
    myLut.at<unsigned char>(i) = clamp_uchar( 255*(i-minValue)/(maxValue - minValue) ); 
...
cv::Mat computeCumulatedHistogramGS(const cv::Mat &imageGS ){
  cv::Mat histogram(256,1,CV_32S,cv::Scalar(0));
  auto histo = computeHistogramGS(imageGS);

  for(int i = 0; i < 256; i++) {
    histogram.at<int>(i) = histogram.at<int>(i-1) + histo.at<int>(i);
  }
...
cv::Mat  generateLUThistogramNormalize(const cv::Mat &imageGS ){
  cv::Mat myLut(256,1,CV_8U);
  auto histo = computeCumulatedHistogramGS(imageGS);

  for(int i=0; i<256; i++)
    myLut.at<unsigned char>(i) = clamp_uchar((255. / (imageGS.cols * imageGS.rows)) * histo.at<int>(i)); 
